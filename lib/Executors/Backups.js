    ...
    return setInterval(() => {
        try {
            ...
            FS.copyFileSync(Connection.Path,
                Path.join(Snapshot > SnapshotLifetime ? BackupDirectory : Snapshots, `${new Date().toISOString()}-${Path.basename(Connection.Path)}`)
            );
            ...
            console.error(`Failed to create a backup of database '${Conncetion.Path}': `, Err.message);
        }
    }, BackupInterval);
}
