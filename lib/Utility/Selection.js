...
class Selection {
     ...
     * @param {Function} predicate Function which either determines the sort order with given arguments `a` and `b`
     * when the preset is arbitrary, or a function to the target property to automatically sort with.
     * @param {Order} [preset] A sort order, an enum from QDB being `ascending`, `descending` or `arbitrary` (default).
     * @returns {Selection}
     */
    order (predicate, preset = QDB.arbitrary) {
        const sortOrderMethod = {
            [QDB.ascending]: (a, b) => predicate(a) - predicate(b),
            [QDB.descending]: (a, b) => predicate(b) - predicate(a),
            [QDB.arbitrary]: predicate
        }[preset];

        this.cache.sort(sortOrderMethod, this);
        return this;
    }
