...
class Selection {
     ...
     * @param {Pathlike} Key Indicates which element to fetch.
    ...
    _CastPath (Frame, Pathlike) {
        ...
        for (let i = 0; i < Pathlike.length; i++) {
            ...
            if (!(Pathlike[i] in Frame))   return undefined;
            Frame = Frame[Pathlike[i]];
        }

        return Frame[LastKey];
    }
     ...
     * @param {Pathlike} KeyOrPath Indicates by which element to group this Selection.
     * @returns {Selection}
     */
    Group (KeyOrPath) {
        ...
        for (const Idx in Original) {
            ...
            const Field = this._CastPath(Row, KeyOrPath.split("."));
