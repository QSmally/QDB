...
class Selection {
     ...
     * @param {Order} [preset] A sort order, either `ascending`, `descending` or `arbitrary` (default).
     * @returns {Selection}
     */
    order (predicate, preset = "arbitrary") {
        const sortOrderMethod = {
            "ascending": (a, b) => predicate(a) - predicate(b),
            "descending": (a, b) => predicate(b) - predicate(a),
            "arbitrary": predicate
        }[preset];
