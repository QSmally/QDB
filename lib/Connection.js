...
class Connection extends BaseConnection {
    ...
    Set (KeyOrPath, Value) {
        ...
        KeyOrPath = this._ResolvePath(KeyOrPath.toString());
        const Key = KeyOrPath instanceof Array ? KeyOrPath[0] : KeyOrPath;
    ...
    Fetch (KeyOrPath) {
        ...
        let Fetched = this.Cache.has(Key) ?
        this.Cache.resolve(Key) :
        JSON.parse(this.API.prepare(`SELECT Val FROM '${this.Table}' WHERE Key = ?;`).get(Key));

        if (typeof Fetched == "undefined") return undefined;
        if (!this.Cache.has(Key)) this.Cache.set(Key, Fetched);
