...
class Connection extends PartialConnection {
    ...
    constructor (PathURL, RawOptions = {}, _Pool = undefined) {
         ...
         * @private
         */
        const Executors = require("../Executors/Client/_Index");
     ...
     * @param {Object|Array} Value The value to set in memory.
    ...
    _Patch (Key, Val) {
        if (this.ValOptions.CacheMaxSize &&
            ...
            const [Key] = this.Cache.reduce((Accumulator, Entry, Key) => {
                if (Entry._Timestamp < Accumulator[1])
    ...
    Select (FnOrPath = () => true, Cache = this.ValOptions.IterCache) {
        ...
        const Entries = typeof FnOrPath === "string" ? this.Fetch(FnOrPath) : (() => {
            ...
            for (const Key of this.Indexes) {
                const Entry = this.Fetch(Key, Cache);
                if (FnOrPath(Entry, Key)) Results[Key] = Entry;
            }
 ...
 * @typedef {Object|Array|Function} DataModel
