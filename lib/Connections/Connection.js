...
class Connection extends PartialConnection {
    ...
    _CastPath (Frame, Pathlike, Item = undefined) {
        ...
        if (Item && typeof Item === "object") delete Item._DataStore;
    ...
    /**
     * Accumulates a function on a row, essentially a fetch wrapped in a method.
     ...
     * @param {Boolean} [Cache] Whether or not to cache the iterating entries.
     * @returns {Connection} Returns the current Connection.
     */
    Each (Fn, Cache = this.ValOptions.IterCache) {
        if (!this._Ready) return null;
        if (typeof Fn !== "function") return null;

        for (const Entry of this.API.prepare(`SELECT * FROM '${this.Table}';`).all())
     ...
     */
    Find (Fn, Cache = this.ValOptions.IterCache) {
        if (!this._Ready) return null;
        if (typeof Fn !== "function") return null;
        ...
        for (const Key of this.Indexes) {
            const Entry = this.Fetch(Key, Cache);
            if (Fn(Entry, Key)) return Entry;
        }
     ...
     * @param {Function|Pathlike} FnOrPath A filter function which returns a boolean, or a row in the database.
     * @param {Boolean} [Cache] Whether or not to cache the iterating entries.
     * @returns {Selection} A Selection class instance.
     */
    Select (FnOrPath = () => true, Cache = this.ValOptions.IterCache) {
        if (!this._Ready) return null;
        ...
        const Entries = typeof FnOrPath === "string" ? this.Fetch(FnOrPath) : (() => {
            ...
            for (const Entry of this.API.prepare(`SELECT * FROM '${this.Table}';`).all())
            if (FnOrPath(Entry.Val, Entry.Key)) Results[Entry.Key] = Entry.Val;
            return Results;
        })();
    ...
    Modify (KeyOrPath, Fn) {
        ...
        const Updated = Fn(this.Fetch(KeyOrPath), Key);
 ...
 * @param {String|Number} _DataStore Identifier of this model.
 * @param {Number} _Timestamp Date when this entry was last patched, used for sweeping of the cache.
 */
