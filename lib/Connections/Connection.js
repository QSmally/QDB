...
class Connection extends PartialConnection {
     ...
     * @param {Pool} [_Pool] Pool reference when this database was initialised in a Pool.
    ...
    ToIntegratedManager (Pathlike = null, Holds = null) {
        ...
        const [Key, Path] = Pathlike ? this._Resolve(Pathlike) : [];
    ...
    Evict (...Keys) {
        ...
        Keys = Keys.map(KeyOrPath => this._Resolve(KeyOrPath)[0]);
    ...
    /**
     * Locally filters out rows in memory. Please note that this method does
     ...
     * @deprecated Method will change in later releases.
     */
    Select (Fn) {
        if (!this._Ready) return null;
        if (typeof Fn !== "function") return null;
     ...
     * This behaves like `if (!DB[Element]) DB[Element] = Input;`.
